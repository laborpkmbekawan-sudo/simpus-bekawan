...
import { catatMutasiAction } from "./actions";
...
export default function FormMutasi({ barangId, namaBarang }: { barangId: string; namaBarang: string }) {
  const [state, formAction] = useFormState(catatMutasiAction, null);
  ...
  return (
    <form ref={formRef} action={formAction} className="flex flex-wrap items-center gap-2">
      ...
      <select name="jenis" defaultValue="masuk" className="rounded-sm border border-sand-100 bg-white px-2 py-1.5 text-xs">
        <option value="masuk">Masuk</option>
        <option value="keluar">Keluar</option>
        <option value="penyesuaian">Penyesuaian (set ke)</option>
      </select>
      <input
        name="jumlah"
        type="number"
      ...
      <input
        name="keterangan"
        placeholder={`Keterangan (${namaBarang})`}
