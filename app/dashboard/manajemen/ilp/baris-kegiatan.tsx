...
import { perbaruiKegiatanIlpAction } from "./actions";
import { LABEL_SIKLUS, LABEL_JENIS_KEGIATAN } from "./form-tambah";
...
export default function BarisKegiatanIlp({
  kegiatan,
  daftarPegawai,
}: {
  kegiatan: KegiatanIlpBaris;
  daftarPegawai: Pegawai[];
}) {
  const [state, formAction] = useFormState(perbaruiKegiatanIlpAction, null);
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <p className="text-xs font-medium text-ink/50">
            {kegiatan.tanggal} · {LABEL_SIKLUS[kegiatan.siklus_hidup] ?? kegiatan.siklus_hidup} ·{" "}
            {LABEL_JENIS_KEGIATAN[kegiatan.jenis_kegiatan] ?? kegiatan.jenis_kegiatan}
          </p>
          <p className="mt-0.5 text-sm font-semibold text-ink">{kegiatan.desa_wilayah}</p>
          {(kegiatan.sasaran != null || kegiatan.capaian != null) && (
            <p className="mt-1 text-xs text-ink/50">
              Sasaran: {kegiatan.sasaran ?? "—"} · Capaian: {kegiatan.capaian ?? "—"}
            </p>
          )}
          {kegiatan.kader_terlibat && <p className="mt-1 text-xs text-ink/50">Kader: {kegiatan.kader_terlibat}</p>}
          {kegiatan.kendala && <p className="mt-1 text-xs text-ink/50">Kendala: {kegiatan.kendala}</p>}
        </div>
